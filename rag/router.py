...
def rule_based_route(query: str) -> Optional[QueryType]:
    ...
    # 分类：身份/能力/架构/技术/使用/对比/内部原理
    _META_PATTERNS = [
        # 身份类：你是谁、你是干嘛的
        r'(你是谁|你是干嘛的|你是做什么的|你是什么(助手|系统|程序)|你的(身份|角色|定位))',
        # 能力类：你能做什么、支持什么功能
        r'(你能(做什么|干啥|回答什么)|你的(功能|能力|作用)|支持哪些(功能|能力)|有什么(功能|能力|用处))',
        r'(支持|能).*(多模态|图文|图片|上传|向量|流式)',
        # 系统指代类：本项目/本系统/这个系统 + 任意描述
        r'(本项目|本系统|这个系统|你的系统|该系统|我们的系统)',
        # 架构/技术类：系统架构、技术栈、技术方案
        r'(系统架构|项目架构|技术架构|系统设计|技术栈|技术方案|技术选型|系统结构)',
        # 知识库类：知识库里有什么、有哪些文件/文档
        r'(知识库|资料库).*(里|中|有|包含|收录|什么)',
        r'(有哪些|有什么|多少个|多少份).*(文件|文档|资料|内容|PDF)',
        # 使用类：如何使用、怎么用
        r'(如何使用|怎么使用|怎么用|怎样用|使用方法|使用说明|使用教程)',
        # 对比类：和其他AI/系统有什么区别
        r'(和其他|与其他|和别的|与别的).*(区别|不同|差异|比较)',
        # 内部原理类：检索原理、embedding、模型、RAG
        r'(检索(原理|机制|策略|方式)|用的什么(模型|embedding|向量)|向量.*(原理|数据库))',
        r'(RAG|rag).*(原理|实现|机制|是什么|怎么做)',
        # 准确性类：回答准不准、可靠吗
        r'(回答.*(准确|可靠|可信)|会不会(出错|答错|幻觉)|准确(率|度|性))',
    ]

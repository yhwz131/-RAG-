...
| **多查询检索 (MQR)** | 跨文档查询自动拆分为多个聚焦子查询，独立检索后融合，来源多样性保证 |
| **查询路由** | LLM-first 路由 + 跨文档路由覆盖（general→rag 强制切换） |
...
└─────────────────────────┬───────────────────────────────┘
     ...
     (跳过检索) (跳过检索)
                          ...
                          ▼
┌─────────────────────────────────────────────────────────┐
│ 3. 查询预处理 + MQR 判断 (chain.py)                     │
│    - preprocess_query() 轻量清洗（去全角、压缩标点）     │
│    - is_cross_doc_query() 判断是否跨文档查询             │
│    - 跨文档 → query_decompose() 拆分子查询               │
│    - 子查询独立检索 + RRF 融合 + 全局去重                │
│    - skip_reranker: 跨文档场景跳过 reranker              │
│    - _diversify_by_source(): 来源多样性保证              │
│    - 非跨文档 → 常规单查询检索                           │
│                                                          │
│ 4. 检索相关文档 (retriever.py)                           │
...
└─────────────────────────┬───────────────────────────────┘
                          ...
                          ▼
┌─────────────────────────────────────────────────────────┐
│ 6. 构建上下文 (chain.py._build_context)                  │
│    - 拼接检索结果为参考资料（带来源标注）                 │
...
│    - FC 结构化输出 + 5 次重试（mimo-v2.5 FC 非确定性）  │
...
└─────────────────────────┬───────────────────────────────┘
                          ...
                          ▼
┌─────────────────────────────────────────────────────────┐
│ 第三层：跨文档路由覆盖（chain.py）                       │
│    - 当 LLM 路由为 general，但 is_cross_doc_query()=True│
│      且查询包含知识库主题关键词 → 强制切换为 rag        │
│    - 防止跨文档查询被误判为 general 导致跳过检索        │
└─────────────────────────┘
```
...
跨文档查询（如"二级Python和三级网络技术分别考什么"）的单一 embedding 会被主导话题"吃掉"，导致只检索到一个文档。MQR 通过查询拆解 + 独立检索解决此问题。

```
用户 Query: "二级Python和三级网络技术分别考什么？"
    │
    ▼
┌─────────────────────────────────────────────────────────┐
│ 1. is_cross_doc_query() 启发式判断                       │
│    - 连接词: 和/与/及/对比/比较/区别/重叠/异同          │
│    - 多问号: "？" ≥ 2 且长度 > 20                        │
│    - 综合模式: "能否应用到"/"结合" 且长度 > 20          │
└─────────────────────────┬───────────────────────────────┘
                          │ (跨文档)
                          ▼
┌─────────────────────────────────────────────────────────┐
│ 2. query_decompose() 查询拆解                            │
│    - FC 结构化输出 (mimo-v2.5) + 5 次重试               │
│    - 输出: ["Python二级考试大纲", "三级网络技术考试大纲"]│
│    - FC 失败 → _rule_based_decompose() 规则兜底         │
│      · 连接词拆分: "A和B" → ["A", "B"]                  │
│      · 综合模式: "A能否应用到B" → ["A", "B核心组件"]    │
│      · 多问号: 按问号拆分                                │
└─────────────────────────┬───────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
│ 3. multi_search() 独立检索 + 融合                        │
│    - 子查询1 → 向量+BM25 → RRF 融合                     │
│    - 子查询2 → 向量+BM25 → RRF 融合                     │
│    - 全局去重 (按 chunk_id)                              │
│    - skip_reranker=True: 跳过 reranker                   │
│      （reranker 用原始查询打分，偏向主查询主题）         │
└─────────────────────────┬───────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
│ 4. _diversify_by_source() 来源多样性选取                 │
│    - 按 source 分组，轮询选取                            │
│    - 确保每个子查询对应的文档都有机会出现在最终结果中    │
│    - 例: [Python大纲, 网络技术大纲, 信息安全大纲]        │
└─────────────────────────────────────────────────────────┘
```
...
| 决策 | 原因 |
|------|------|
| 跳过 reranker | reranker 用原始查询打分，LangChain 等高频词会压过次要主题 |
| 规则兜底 | mimo-v2.5 FC 非确定性，5 次重试仍可能失败 |
| 来源多样性 | RRF 融合后同一文档多个切片排名靠前，轮询避免单一来源 |
| 跨文档路由覆盖 | LLM 路由可能将跨文档查询误判为 general，导致跳过检索 |

**评测结果 (32 用例)：**

| 指标 | 基线 | MQR 后 |
|------|------|--------|
| 总通过率 | 90.6% (29/32) | **93.8% (30/32)** |
| cross_doc 类型 | 60% (3/5) | **100% (5/5)** |
| reject 类型 | 100% (7/7) | 100% (7/7) |
...
# ========== 跨文档多查询检索 (MQR) ==========
MULTI_QUERY_ENABLED=true    # 是否启用 MQR
MULTI_QUERY_MAX_SUB=3       # 最多拆分几个子查询
...
| 跨文档查询误判为 general | chain.py 跨文档路由覆盖：is_cross_doc_query() + 主题关键词 → 强制 rag |
| MQR FC 拆解失败 | 5 次重试 + _rule_based_decompose() 规则兜底，最终降级为单查询 |
